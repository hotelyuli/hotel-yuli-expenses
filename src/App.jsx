...
function ReconcileDashboard({ data, db, showToast }) {
  ...
  // Accountant totals — total_general is always CRC in this format
  const acctCRCrecs = monthAcctRecs.filter((r) => r.currency === "CRC");
  const acctUSDrecs = monthAcctRecs.filter((r) => r.currency === "USD");
  const totalAcctCRC = acctCRCrecs.reduce((s, r) => s + (r.total_crc || 0), 0);
  // For USD records, derive original USD = total_crc / exchange_rate
  const totalAcctUSD = acctUSDrecs.reduce((s, r) => s + ((r.total_crc || 0) / (r.exchange_rate || 1)), 0);
