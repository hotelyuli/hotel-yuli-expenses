...
function parseAccountantRows(jsonRows, uploadId, uploadedBy) {
  return jsonRows
    ...
    .map((r) => {
      ...
      const total = parseFloat(String(r["Total General"] || r["total_general"] || "0").replace(/,/g, "")) || 0;
      ...
      return {
        ...
        total_general: total,
        total_crc: total,  // Total General is always already in CRC in this format
        subtotal: parseFloat(r["Subtotal"] || 0) || 0,
        total_impuestos: parseFloat(r["Total Impuestos"] || 0) || 0,
        uploaded_by: uploadedBy,
        uploaded_at: new Date().toISOString(),
      };
    });
...
function ReconcileDashboard({ data, db, showToast }) {
  ...
  const handleAcctFile = async (e) => {
    ...
    try {
      const arrayBuffer = await file.arrayBuffer();
      // cellFormula:false + raw:false forces SheetJS to use cached formula results
