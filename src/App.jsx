  ...
  const handleBankFile = async (e) => {
    ...
    try {
      ...
      if (fileName.endsWith(".csv")) {
        ...
        // XLS / HTML path — read as text first
        const text = await file.text();
        // Check if it's HTML (BCR exports XLS that is really HTML)
        if (text.includes("<table") || text.includes("<td") || text.includes("<TR") || text.includes("<td")) {
          rows = parseBankStatementHTML(text, bank, uploadId, "Yusrei");
        } else {
          // Try SheetJS for real XLSX
