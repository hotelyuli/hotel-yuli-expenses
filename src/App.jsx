...
function parseBankStatementHTML(htmlText, bank, uploadId, uploadedBy) {
  try {
    ...
    allTrs.forEach((tr) => {
      ...
      const cells = Array.from(tds).map((td) => td.textContent.replace(/\u00a0/g, "").trim());

      // Find the date cell — BCR format has 6 tds: [fechaContable, fechaTrans, hora, doc, desc, debito]
      // fechaContable is often empty, fechaTrans has the date
      let dateStr = null;
      let docNum = null;
      let desc = null;
      let amountStr = null;
      ...
      for (let i = 0; i < cells.length; i++) {
        if (/^\d{2}\/\d{2}\/\d{4}$/.test(cells[i])) {
          dateStr = cells[i];
          // Based on BCR structure: date@i, time@i+1, doc@i+2, desc@i+3, amount@i+4
          docNum = cells[i + 2] || "";
          desc = cells[i + 3] || "";
