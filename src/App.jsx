...
function parseBankStatementHTML(htmlText, bank, uploadId, uploadedBy) {
  try {
    ...
    const accountCurrency = (bodyText.includes("Dólares") || bodyText.includes("USD") || bodyText.includes("dolares")) ? "USD" : "CRC";
    ...
    allTrs.forEach((tr) => {
      ...
      // Get text content of each cell, strip &nbsp; and whitespace
      ...
      // Find the date cell — use non-anchored match to handle any leading/trailing space
      let dateStr = null;
      let docNum = null;
      let desc = null;
      let amountStr = null;
      ...
      for (let i = 0; i < cells.length; i++) {
        ...
        if (match) {
          dateStr = match[1]; // extracted date only
          docNum = cells[i + 2] || "";
          desc = cells[i + 3] || "";
      ...
      // Parse amount — remove commas, spaces, handle negative
      const rawAmount = amountStr.replace(/,/g, "").replace(/\s/g, "");
