...
function parseAccountantRows(jsonRows, uploadId, uploadedBy) {
  return jsonRows
    .filter((r) => {
      // Accept row if it has a vendor name (non-empty string)
    ...
    .map((r) => {
      ...
      const rate = parseFloat(String(r["Tipo de Cambio"] || r["tipo_de_cambio"] || 1).replace(/,/g, "")) || 1;

      // Total General may be a formula — use Subtotal + Total Impuestos as reliable fallback
      const rawTotal = r["Total General"] || r["total_general"];
      const subtotal = parseFloat(String(r["Subtotal"] || 0).replace(/,/g, "")) || 0;
      const impuestos = parseFloat(String(r["Total Impuestos"] || 0).replace(/,/g, "")) || 0;
      const parsedTotal = parseFloat(String(rawTotal || "0").replace(/,/g, "")) || 0;
      // If parsedTotal is 0 but subtotal exists, compute from components
      const total = parsedTotal > 0 ? parsedTotal : (subtotal + impuestos);

      // Handle Fecha — can be JS Date object, string, or Excel serial number
      let fechaStr = null;
      const rawFecha = r["Fecha"] || r["fecha"];
      if (rawFecha instanceof Date) {
        fechaStr = rawFecha.toISOString().slice(0, 10);
      } else if (typeof rawFecha === "string" && rawFecha.length >= 10) {
        fechaStr = rawFecha.slice(0, 10);
      ...
      return {
        id: genId(),
        upload_id: uploadId,
        document_number: String(r["Número de Documento"] || r["numero_de_documento"] || "").trim() || null,
        currency,
        exchange_rate: rate,
        ...
        id_proveedor: String(r["ID Proveedor"] || r["ID Proveedor"] || "").trim() || null,
        total_general: total,
        total_crc: total,
        subtotal: subtotal,
...
function ReconcileDashboard({ data, db, showToast }) {
  ...
  const handleAcctFile = async (e) => {
    ...
    try {
      ...
      const ws = wb.Sheets[wb.SheetNames[0]];
      // Get raw rows with formula values (v = cached value)
