...
const CATEGORY_GROUPS = [
  {
    group: "People",
    items: ["Payroll", "Employment taxes and CCSS", "Contractors and professional services"],
  },
  {
  ...
  },
  {
    ...
    items: ["Accounting and legal", "Insurance", "Taxes, permits and municipal fees", "Software and subscriptions"],
  },
  {
    group: "Sales and Finance",
    items: ["Marketing and advertising", "OTA commissions", "Bank and card fees"],
  },
  {
    group: "Travel and Investment",
    items: ["Travel expenses", "Furniture, equipment and capital purchases", "Other expenses"],
  },
];
...
function rowToExpense(r) {
  return {
    ...
    scanConfidence: r.scan_confidence,
  };
}
...
function expenseToRow(e) {
  return {
    ...
    subtotal: e.subtotal === "" || e.subtotal == null ? null : Number(e.subtotal),
    tax_amount: e.taxAmount === "" || e.taxAmount == null ? null : Number(e.taxAmount),
    document_type: e.documentType || null,
    scan_confidence: e.scanConfidence || null,
  };
}
...
function parseBankStatementHTML(htmlText, bank, uploadId, uploadedBy) {
  try {
    const accountCurrency = (htmlText.includes("D\u00f3lares") || htmlText.includes("dolares") || htmlText.includes("USD")) ? "USD" : "CRC";

    // Extract all TD content using split — avoids stateful regex issues
    const allCells = htmlText
      .split(/<td[^>]*>/i)
      .slice(1)
      .map((chunk) => {
        const end = chunk.indexOf("</td>");
    ...
    for (let i = 0; i < allCells.length; i++) {
      const dateMatch = allCells[i].match(/(\d{2}\/\d{2}\/\d{4})/);
      if (dateMatch) {
        const dateStr = dateMatch[1];
        const doc  = allCells[i + 2] || "";   // i+1 = time, i+2 = doc number
        const desc = allCells[i + 3] || "";
        const amountRaw = allCells[i + 4] || "";
        const amount = Math.abs(parseFloat(amountRaw.replace(/,/g, "").replace(/\s/g, "")) || 0);
        if (amount > 0) {
          rows.push({
            id: genId(),
            upload_id: uploadId,
            ...
            exchange_rate: accountCurrency === "CRC" ? 1 : null,
            amount_crc: accountCurrency === "CRC" ? amount : null,
...
// Parse BCR bank statement exported as CSV
function parseBankCSV(csvText, bank, uploadId, uploadedBy) {
  const lines = csvText.split(/\r?\n/).filter((l) => l.trim());
  const rows = [];
  const bodyText = csvText.toLowerCase();
  const accountCurrency = (bodyText.includes("dólares") || bodyText.includes("dolares") || bodyText.includes("usd")) ? "USD" : "CRC";
  ...
  for (const line of lines) {
    ...
    for (let i = 0; i < cells.length; i++) {
      const match = cells[i].match(/(\d{2}\/\d{2}\/\d{4})/);
      if (match) {
        dateStr = match[1];
        desc = cells[i + 3] || cells[i + 2] || "";
        amountStr = cells[i + 4] || cells[cells.length - 1] || "0";
        break;
      }
    }
    if (!dateStr) continue;
    const amount = Math.abs(parseFloat(amountStr.replace(/,/g, "").replace(/\s/g, "")) || 0);
    if (amount === 0) continue;
    rows.push({
      id: genId(), upload_id: uploadId, bank, account_currency: accountCurrency,
      transaction_date: dateStr, document_number: "", description: desc,
      amount_original: amount,
      exchange_rate: accountCurrency === "CRC" ? 1 : null,
      amount_crc: accountCurrency === "CRC" ? amount : null,
      matched_expense_id: null, uploaded_by: uploadedBy, uploaded_at: new Date().toISOString(),
    });
  }
  return rows;
}

// Parse BCR statement read via SheetJS (real Excel)
function parseBankSheetRows(jsonRows, bank, uploadId, uploadedBy) {
  const rows = [];
  let accountCurrency = "CRC";
  // Check header rows for currency info
  for (const row of jsonRows.slice(0, 10)) {
    const rowStr = row.join(" ").toLowerCase();
    if (rowStr.includes("dólares") || rowStr.includes("dolares") || rowStr.includes("usd")) {
      accountCurrency = "USD"; break;
    }
  }
  for (const cells of jsonRows) {
    let dateStr = null, desc = null, amountStr = null;
    for (let i = 0; i < cells.length; i++) {
      const cellStr = String(cells[i] || "").trim();
      const match = cellStr.match(/(\d{2}\/\d{2}\/\d{4})/);
      if (match) {
        dateStr = match[1];
        desc = String(cells[i + 3] || cells[i + 2] || "").trim();
        amountStr = String(cells[i + 4] || cells[cells.length - 1] || "0").trim();
        break;
      }
    }
    if (!dateStr) continue;
    const amount = Math.abs(parseFloat(String(amountStr).replace(/,/g, "")) || 0);
    if (amount === 0) continue;
    rows.push({
      id: genId(), upload_id: uploadId, bank, account_currency: accountCurrency,
      transaction_date: dateStr, document_number: "", description: desc,
...
// Parse accountant Excel (SheetJS JSON output)
// Note: Total General column contains Excel formulas like =1921*C2
// We extract the base amount from the formula and multiply by Tipo de Cambio
function parseAccountantRows(jsonRows, uploadId, uploadedBy) {
  return jsonRows
    .filter((r) => {
      const proveedor = r["Proveedor"] || r["proveedor"] || "";
      return String(proveedor).trim().length > 0;
    })
    .map((r) => {
      const currency = String(r["Moneda"] || r["moneda"] || "CRC").trim();
      const rate = parseFloat(String(r["Tipo de Cambio"] || 1).replace(/,/g, "")) || 1;

      // Total General: for CRC rows it's a plain number; for USD rows it may be a formula string
      // We compute total_crc = base_amount * exchange_rate
      let total = 0;
      const rawTotal = r["Total General"] || r["total_general"];

      if (typeof rawTotal === "number" && rawTotal !== 0) {
        // Plain number (CRC rows) — already in CRC
        total = Math.abs(rawTotal);
      } else if (typeof rawTotal === "string" && rawTotal.includes("=")) {
        // Formula like =1921*C2 — extract the numeric base and multiply by rate
        const nums = rawTotal.replace(/=/g, "").split(/[*\/+\-]/).map(s => parseFloat(s.trim())).filter(n => !isNaN(n) && n > 1);
        const base = nums.length > 0 ? Math.max(...nums) : 0;
        total = base * rate;
      } else {
        // Fallback: parse as number directly
        total = Math.abs(parseFloat(String(rawTotal || "0").replace(/,/g, "")) || 0);
      }
      ...
      } else if (typeof rawFecha === "string" && rawFecha.length >= 8) {
        // Could be M/D/YYYY from raw:false or YYYY-MM-DD
        const parts = rawFecha.match(/(\d+)[\/-](\d+)[\/-](\d+)/);
        if (parts) {
          const [, a, b, c] = parts;
          fechaStr = c.length === 4 ? `${c}-${b.padStart(2,"0")}-${a.padStart(2,"0")}` : rawFecha.slice(0, 10);
        }
      ...
      return {
        id: genId(),
        upload_id: uploadId,
        document_number: String(r["Número de Documento"] || "").trim() || null,
        currency,
        exchange_rate: rate,
        document_type: String(r["Tipo de Documento"] || "").trim() || null,
        transaction_date: fechaStr,
        receptor: String(r["Receptor"] || "").trim() || null,
        proveedor: String(r["Proveedor"] || "").trim() || null,
        id_proveedor: String(r["ID Proveedor"] || "").trim() || null,
        total_general: total,
        total_crc: total,
        subtotal: 0,
        total_impuestos: 0,
...
- Número de factura / consecutivo / comprobante
...
Responde SOLO con un objeto JSON (sin markdown, sin backticks, sin explicación):
{
  ...
  "documentType": "electronic_invoice", "receipt", "bank_transfer", "payroll", "utility_bill" o "other",
...
Reglas de categoría:
- Planilla/nómina/salario → "Payroll"
- CCSS/Caja Costarricense → "Employment taxes and CCSS"
- Contador, abogado, consultor o contratista → "Contractors and professional services" o "Accounting and legal"
- Electricidad, agua, gas, internet o teléfono → "Utilities"
- Limpieza, lavandería o ama de llaves → "Cleaning and housekeeping"
- Supermercado, desayuno, amenities o suministros para huéspedes → "Guest and breakfast supplies"
- Piscina, jardinería o químicos de piscina → "Pool and garden maintenance"
- Ferretería, aire acondicionado, reparación o mantenimiento → "Maintenance and repairs"
- Recolección de residuos o servicios sanitarios → "Waste and sanitation"
- Parqueo, taxi o transporte local → "Parking and transportation"
- Seguro o póliza → "Insurance"
- Hacienda, municipalidad, patente o permiso → "Taxes, permits and municipal fees"
- Google Workspace, software o suscripción → "Software and subscriptions"
- Google Ads, Meta o publicidad → "Marketing and advertising"
- Booking.com, Expedia u otra OTA → "OTA commissions"
- Datáfono, comisión bancaria o fee por transacción → "Bank and card fees"
- Vuelo, hotel, alquiler de vehículo o viático → "Travel expenses"
- Mueble, equipo duradero o mejora de capital → "Furniture, equipment and capital purchases"
- Si no hay suficiente información → "Other expenses"
...
Si un campo no se puede determinar, usa null. Si la imagen está borrosa o ilegible, intenta extraer lo que puedas y usa confidence "low".` },
  ...
  try {
    ...
    return { amount: null, currency: null, vendor: null, invoiceNumber: null, subtotal: null, taxAmount: null, documentType: null, date: null, category: null, description: null, confidence: "low" };
  }
}
            ...
            <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
              <input
                ...
                onChange={(e) => {
                  ...
                  setPin(val);
                  // Auto-login as soon as 4 digits entered and correct
                  if (val.length === 4) {
                    if (val === ADMIN_PIN) {
                      onLogin(selected, "admin");
                    } else {
                      setPinError(true);
                      setTimeout(() => { setPinError(false); setPin(""); }, 1200);
                    }
                  }
                }}
...
function SubmitExpense({ data, db, staffName, showToast }) {
  ...
  const [form, setForm] = useState({
    ...
    invoiceNumber: "", subtotal: "", taxAmount: "", documentType: "",
  ...
  const handleFile = async (e) => {
    ...
    reader.onload = async (ev) => {
      ...
      try {
        ...
        if (result.invoiceNumber) { newForm.invoiceNumber = result.invoiceNumber; filledFields.add("invoiceNumber"); }
        if (result.subtotal != null) { newForm.subtotal = String(result.subtotal); filledFields.add("subtotal"); }
        if (result.taxAmount != null) { newForm.taxAmount = String(result.taxAmount); filledFields.add("taxAmount"); }
        if (result.documentType) { newForm.documentType = result.documentType; filledFields.add("documentType"); }
        ...
        reader.onload = async (ev) => {
          ...
          try {
            ...
            const expense = {
              ...
              status: staffName === "Yusrei" ? "approved" : "pending",
  ...
  const handleSubmit = async (withWhatsApp = true) => {
    ...
    const expense = {
      ...
      status: staffName === "Yusrei" ? "approved" : "pending", invoiceAttached: !!invoiceData,
  ...
  const resetForm = () => {
    setForm({ amount: "", currency: "USD", category: "", description: "", bankAccount: "", paymentMethod: "Transfer", vendor: "", invoiceNumber: "", subtotal: "", taxAmount: "", documentType: "", date: new Date().toISOString().slice(0, 10) });
    ...
    <div>
      ...
      {mode === "single" && (
        <div>
          ...
          <div style={{ display: "flex", gap: "10px", marginBottom: "14px" }}>
            <div style={{ flex: 1 }}>
              {aiLabel("invoiceNumber", "Invoice number")}
              <input type="text" value={form.invoiceNumber} onChange={(e) => setForm({ ...form, invoiceNumber: e.target.value })}
                placeholder="Consecutivo" style={{ ...fieldStyle, ...aiHighlight("invoiceNumber") }} />
            </div>
            <div style={{ flex: 1 }}>
              {aiLabel("taxAmount", "Tax / IVA")}
              <input type="text" inputMode="decimal" value={form.taxAmount} onChange={(e) => setForm({ ...form, taxAmount: e.target.value })}
                placeholder="0.00" style={{ ...fieldStyle, ...aiHighlight("taxAmount") }} />
            </div>
          </div>
...
function MonthlyReport({ data, db, showToast }) {
  const [selectedMonth, setSelectedMonth] = useState(getMonthKey(new Date().toISOString()));
  const [rate, setRate] = useState(520); // CRC per USD
  ...
  // Totals per currency
  const totalCRC = approved.filter(e => e.currency === "CRC").reduce((s, e) => s + e.amount, 0);
  const totalUSD = approved.filter(e => e.currency === "USD").reduce((s, e) => s + e.amount, 0);
  const totalUSDequiv = totalUSD + (totalCRC / rate); // everything in USD

  // Category breakdown — group by category, split CRC/USD
  const byCategory = {};
  approved.forEach((e) => {
    if (!byCategory[e.category]) byCategory[e.category] = { crc: 0, usd: 0, count: 0 };
    if (e.currency === "CRC") byCategory[e.category].crc += e.amount;
    else byCategory[e.category].usd += e.amount;
    byCategory[e.category].count++;
  });
  const categoryList = Object.entries(byCategory)
    .map(([cat, v]) => ({ cat, ...v, totalUSD: v.usd + v.crc / rate }))
  ...
  const exportCSV = () => {
    ...
    const rows = approved.map((e) => {
      ...
      return [e.date, e.amount, e.currency, amtUSD.toFixed(2), e.category, e.vendor || "", e.paymentMethod, e.bankAccount || "", e.submittedBy, e.status, e.invoiceAttached ? "Yes" : "No", e.aiScanned ? "Yes" : "No", (e.description || "").replace(/,/g, ";")];
    });
  ...
  return (
    <div>
      ...
      {/* Month selector — fixed Jan-Dec 2026 */}
      <div style={{ marginBottom: "12px", position: "relative" }}>
        <select value={selectedMonth} onChange={(e) => setSelectedMonth(e.target.value)} style={{
          width: "100%", padding: "10px 14px", borderRadius: "10px", border: `1.5px solid ${B.blushDark}`,
          fontSize: "14px", background: B.white, color: B.textDark, appearance: "none", WebkitAppearance: "none", paddingRight: "30px"
        }}>
          {FIXED_MONTHS.map((mk) => { const [y, m] = mk.split("-"); return <option key={mk} value={mk}>{MONTHS[parseInt(m) - 1]} {y}</option>; })}
        </select>
        <div style={{ position: "absolute", right: "10px", top: "50%", transform: "translateY(-50%)", pointerEvents: "none", color: B.textLight }}><Icons.ChevronDown /></div>
      </div>

      {/* Exchange rate */}
      <div style={{ ...cardStyle, display: "flex", alignItems: "center", gap: "12px", padding: "10px 14px", marginBottom: "12px" }}>
        <span style={{ fontSize: "12px", fontWeight: 600, color: B.textMid, whiteSpace: "nowrap" }}>Rate ₡/$</span>
        <input type="number" value={rate} onChange={(e) => setRate(parseFloat(e.target.value) || 520)}
          style={{ flex: 1, padding: "6px 10px", borderRadius: "8px", border: `1.5px solid ${B.blushDark}`, fontSize: "14px", fontWeight: 600, color: B.darkWine, background: B.cream, outline: "none" }} />
        <span style={{ fontSize: "11px", color: B.textLight, whiteSpace: "nowrap" }}>CRC per $1</span>
      </div>

      {/* KPI row */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "10px", marginBottom: "16px" }}>
        <div style={{ ...cardStyle, textAlign: "center", marginBottom: 0 }}>
          <div style={{ fontSize: "10px", color: B.textLight, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: "6px" }}>CRC Total</div>
          <div style={{ fontSize: "16px", fontWeight: 700, color: B.darkWine }}>{formatCurrency(totalCRC, "CRC")}</div>
        </div>
        <div style={{ ...cardStyle, textAlign: "center", marginBottom: 0 }}>
          <div style={{ fontSize: "10px", color: B.textLight, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: "6px" }}>USD Total</div>
          <div style={{ fontSize: "16px", fontWeight: 700, color: B.darkWine }}>{formatCurrency(totalUSD, "USD")}</div>
        </div>
        <div style={{ ...cardStyle, textAlign: "center", marginBottom: 0, background: B.blush }}>
          <div style={{ fontSize: "10px", color: B.textLight, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.05em", marginBottom: "6px" }}>Total in USD</div>
          <div style={{ fontSize: "16px", fontWeight: 700, color: B.darkWine }}>${totalUSDequiv.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
        </div>
      </div>
      ...
      <div style={{ ...cardStyle, marginBottom: "16px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: "13px" }}>
          <span style={{ color: B.greenDark, fontWeight: 600 }}>✓ {approved.length} approved</span>
          <span style={{ color: "#D4890B", fontWeight: 600 }}>◷ {pending.length} pending</span>
          {missingInvoice.length > 0 && <span style={{ color: B.red, fontWeight: 600 }}>! {missingInvoice.length} missing invoice</span>}
        </div>
      </div>
      ...
      {categoryList.length > 0 && (
        <div style={cardStyle}>
          ...
          {categoryList.map((c, i) => {
            const pct = (c.totalUSD / maxCatUSD) * 100;
            return (
              <div key={i} style={{ marginBottom: "12px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", fontSize: "12px", marginBottom: "3px" }}>
                  <span style={{ color: B.textMid }}>{c.cat} <span style={{ color: B.textLight }}>({c.count})</span></span>
                  <div style={{ display: "flex", gap: "8px", flexShrink: 0 }}>
                    {c.crc > 0 && <span style={{ fontWeight: 600, color: B.darkWine }}>{formatCurrency(c.crc, "CRC")}</span>}
                    {c.usd > 0 && <span style={{ fontWeight: 600, color: B.terracottaDark }}>{formatCurrency(c.usd, "USD")}</span>}
                    <span style={{ fontWeight: 700, color: B.textLight, fontSize: "11px" }}>${c.totalUSD.toLocaleString("en-US", { maximumFractionDigits: 0 })}</span>
                  </div>
                </div>
                <div style={{ height: "6px", borderRadius: "3px", background: B.blush, display: "flex", overflow: "hidden" }}>
                  <div style={{ height: "100%", background: B.darkWine, width: `${pct * (c.crc / (c.crc + c.usd * rate || 1))}%` }} />
                  <div style={{ height: "100%", background: B.terracotta, width: `${pct * (c.usd * rate / (c.crc + c.usd * rate || 1))}%` }} />
                </div>
              </div>
            );
          })}
          <div style={{ fontSize: "12px", fontWeight: 700, color: B.textDark, marginTop: "12px", paddingTop: "10px", borderTop: `1px solid ${B.blush}`, display: "flex", justifyContent: "space-between" }}>
            <span>Total</span>
            <span>${totalUSDequiv.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} USD</span>
          </div>
        </div>
      )}
      ...
      {approved.length > 0 && <div style={{ fontSize: "11px", color: B.textLight, textAlign: "center", marginTop: "8px" }}>Includes USD equivalent column · Rate: ₡{rate}/$1</div>}
    </div>
  );
}
...
function ReconcileDashboard({ data, db, showToast }) {
  ...
  const [selectedMonth, setSelectedMonth] = useState(getMonthKey(new Date().toISOString()));
  const [monthRate, setMonthRate] = useState(520); // CRC/USD reference rate
  ...
  const handleBankFile = async (e) => {
    ...
    try {
      ...
      if (fileName.endsWith(".csv")) {
        ...
        rows = parseBankCSV(text, bank, uploadId, "Yusrei");
      } else {
        const text = await file.text();
        const hasHTML = text.includes("<table") || text.includes("<td") || text.includes("<TD") || text.includes("<TR") || text.includes("<tr");
        const dateCount = (text.match(/\d{2}\/\d{2}\/\d{4}/g) || []).length;
        console.log("Bank file debug:", { fileName, hasHTML, dateCount, firstChars: text.slice(0, 200) });
        setUploadMsg(`File: ${file.name} | HTML: ${hasHTML} | Dates found: ${dateCount}`);
        await new Promise(r => setTimeout(r, 2000)); // show debug message

        if (hasHTML) {
          rows = parseBankStatementHTML(text, bank, uploadId, "Yusrei");
          console.log("HTML parser result:", rows.length, "rows");
        } else {
          const arrayBuffer = await file.arrayBuffer();
          const wb = XLSX.read(arrayBuffer, { type: "array" });
          const ws = wb.Sheets[wb.SheetNames[0]];
          const jsonRows = XLSX.utils.sheet_to_json(ws, { header: 1, defval: "" });
          console.log("SheetJS rows:", jsonRows.length, "first row:", jsonRows[0]);
          rows = parseBankSheetRows(jsonRows, bank, uploadId, "Yusrei");
        }
      }

      if (rows.length === 0) {
        showToast(`No transactions found — try exporting as CSV from BCR/LAFISE online banking`, "error");
        setUploading(false); return;
      }

      // Fetch exchange rates for USD transactions
      const usdRows = rows.filter(r => r.account_currency === "USD" && !r.amount_crc);
      if (usdRows.length > 0) {
        setUploadMsg(`Fetching exchange rates for ${usdRows.length} USD transactions...`);
        const rateCache = {};
  ...
  const handleAcctFile = async (e) => {
    ...
    try {
      ...
      // Read cell range manually to get raw .v values (bypasses formula strings)
      const range = XLSX.utils.decode_range(ws["!ref"] || "A1:W500");
      const headers = [];
      const jsonRows = [];
      ...
      for (let R = range.s.r; R <= range.e.r; R++) {
        ...
        for (let C = range.s.c; C <= range.e.c; C++) {
          ...
          if (R === range.s.r) {
            ...
            if (!header) continue;
            // Use .v for cached value, fall back to .w (formatted string)
            let val = null;
            if (cell) {
              if (cell.t === "n") val = cell.v;           // number
              else if (cell.t === "d") val = cell.v;      // date
              else if (cell.t === "s") val = cell.v;      // string
              else if (cell.t === "f") val = cell.v ?? (cell.w ? parseFloat(cell.w.replace(/,/g,"")) || cell.w : null); // formula: use cached v
              else val = cell.w || cell.v || null;
            }
            if (val !== null && val !== undefined && val !== "") hasData = true;
            rowObj[header] = val;
          }
        }
        if (R > range.s.r && hasData) jsonRows.push(rowObj);
      }
  ...
  // Bank totals converted to USD (CRC ÷ monthRate, USD stays)
  ...
  const totalBankCRC_orig = bankCRCtxns.reduce((s, t) => s + (t.amount_original || 0), 0);
  const totalBankUSD_orig = bankUSDtxns.reduce((s, t) => s + (t.amount_original || 0), 0);
  const totalBankUSD = totalBankUSD_orig + (totalBankCRC_orig / monthRate);

  // Accountant totals converted to USD
  // total_crc is always CRC; USD records have exchange_rate stored
  const acctCRCrecs = monthAcctRecs.filter((r) => r.currency === "CRC");
  const acctUSDrecs = monthAcctRecs.filter((r) => r.currency === "USD");
  const totalAcctCRC_orig = acctCRCrecs.reduce((s, r) => s + (r.total_crc || 0), 0);
  const totalAcctUSD_orig = acctUSDrecs.reduce((s, r) => s + ((r.total_crc || 0) / (r.exchange_rate || monthRate)), 0);
  const totalAcctUSD = totalAcctUSD_orig + (totalAcctCRC_orig / monthRate);

  // Gap in USD — positive = bank paid more than accountant received
  const gapUSD = totalBankUSD - totalAcctUSD;

  // Also keep CRC breakdown for display
  const totalBankCRC = totalBankCRC_orig;
  const totalAcctCRC = totalAcctCRC_orig;
  ...
  const totalExpUSD_orig = expUSDlist.reduce((s, e) => s + e.amount, 0);
  const totalExpUSD = totalExpUSD_orig + (totalExpCRC / monthRate);
  const totalExpAllCRC = totalExpCRC + totalExpUSD_orig * monthRate;

  // Find bank transactions not matched to any accountant record
  // Both sides now in CRC — compare bank amount_crc vs accountant total_crc
  const acctAmountsCRC = new Set(acctCRCrecs.map((r) => Math.round(r.total_crc || 0)));
  const acctAmountsUSD = new Set(acctUSDrecs.map((r) => Math.round((r.total_crc || 0) / (r.exchange_rate || 1) * 100) / 100));
  ...
  // Fixed month list: Jan 2026 - Dec 2026
  const allMonths = [
    "2026-01","2026-02","2026-03","2026-04","2026-05","2026-06",
    "2026-07","2026-08","2026-09","2026-10","2026-11","2026-12",
  ];
      ...
      {activeTab === "dashboard" && !uploading && (
        <div>
          ...
          {loading ? (
            ...
            <>
              {/* Exchange rate input */}
              <div style={{ ...cardS, display: "flex", alignItems: "center", gap: "12px", padding: "12px 16px" }}>
                <div style={{ fontSize: "12px", fontWeight: 600, color: B.textMid, whiteSpace: "nowrap" }}>Exchange Rate ₡/$</div>
                <input
                  type="number"
                  value={monthRate}
                  onChange={(e) => setMonthRate(parseFloat(e.target.value) || 520)}
                  style={{ flex: 1, padding: "8px 12px", borderRadius: "8px", border: `1.5px solid ${B.blushDark}`, fontSize: "14px", fontWeight: 600, color: B.darkWine, background: B.cream, outline: "none" }}
                />
                <div style={{ fontSize: "11px", color: B.textLight, whiteSpace: "nowrap" }}>CRC per $1 USD</div>
              </div>

              {/* Summary — USD as primary, CRC breakdown */}
              {[
                {
                  label: "Bank Outflows",
                  usdTotal: totalBankUSD,
                  crcPart: totalBankCRC, usdPart: totalBankUSD_orig,
                  sub: `${bankCRCtxns.length} CRC txns · ${bankUSDtxns.length} USD txns`,
                  color: B.darkWine, bg: B.blush,
                },
                {
                  label: "Accountant Received",
                  usdTotal: totalAcctUSD,
                  crcPart: totalAcctCRC, usdPart: totalAcctUSD_orig,
                  sub: `${acctCRCrecs.length} CRC invoices · ${acctUSDrecs.length} USD invoices`,
                  color: B.greenDark, bg: B.greenLight,
                },
                {
                  label: gapUSD > 1 ? "⚠ Gap (Not Reported)" : "✓ Balanced",
                  usdTotal: Math.abs(gapUSD),
                  crcPart: Math.abs(totalBankCRC - totalAcctCRC),
                  usdPart: Math.abs(totalBankUSD_orig - totalAcctUSD_orig),
                  sub: gapUSD > 1 ? `$${fmtUSD(gapUSD)} not accounted for` : "Bank and accountant match",
                  color: gapUSD > 1 ? B.red : B.greenDark,
                  bg: gapUSD > 1 ? B.redLight : B.greenLight,
                },
              ].map((row) => (
                <div key={row.label} style={{ ...cardS, background: row.bg, border: `1px solid ${B.blushDark}` }}>
                  <div style={{ fontSize: "11px", fontWeight: 700, color: row.color, textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: "8px" }}>{row.label}</div>

                  {/* Primary: TOTAL IN USD */}
                  <div style={{ fontSize: "28px", fontWeight: 700, color: row.color, marginBottom: "10px", borderBottom: `1px solid rgba(0,0,0,0.06)`, paddingBottom: "10px" }}>
                    {fmtUSD(row.usdTotal)}
                  </div>

                  {/* Breakdown row */}
                  <div style={{ display: "flex", gap: "8px", marginBottom: "6px" }}>
                    <div style={{ flex: 1, background: B.white, borderRadius: "8px", padding: "8px 10px" }}>
                      <div style={{ fontSize: "10px", color: B.textLight, fontWeight: 600, marginBottom: "3px" }}>₡ CRC</div>
                      <div style={{ fontSize: "14px", fontWeight: 700, color: row.color }}>{fmtCRC(row.crcPart)}</div>
                      <div style={{ fontSize: "10px", color: B.textLight, marginTop: "1px" }}>≈ {fmtUSD(row.crcPart / monthRate)}</div>
                    </div>
                    <div style={{ flex: 1, background: B.white, borderRadius: "8px", padding: "8px 10px" }}>
                      <div style={{ fontSize: "10px", color: B.textLight, fontWeight: 600, marginBottom: "3px" }}>$ USD</div>
                      <div style={{ fontSize: "14px", fontWeight: 700, color: row.color }}>{fmtUSD(row.usdPart)}</div>
                      <div style={{ fontSize: "10px", color: B.textLight, marginTop: "1px" }}>≈ {fmtCRC(row.usdPart * monthRate)}</div>
                    </div>
                  </div>

                  <div style={{ fontSize: "11px", color: row.color, opacity: 0.8 }}>{row.sub}</div>
                </div>
              ))}

              {/* Grand total USD summary */}
              {(monthBankTxns.length > 0 || monthAcctRecs.length > 0) && (
                <div style={{ ...cardS, background: B.darkWine, border: "none", marginBottom: "16px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      <div style={{ fontSize: "11px", color: "rgba(255,255,255,0.6)", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em" }}>Gap (Bank − Accountant)</div>
                      <div style={{ fontSize: "11px", color: "rgba(255,255,255,0.5)", marginTop: "2px" }}>at ₡{monthRate} per $1</div>
                    </div>
                    <div style={{ textAlign: "right" }}>
                      <div style={{ fontSize: "24px", fontWeight: 700, color: gapUSD > 1 ? "#FF9999" : "#99FFB3" }}>
                        {gapUSD > 0 ? "+" : ""}{fmtUSD(gapUSD)}
                      </div>
                      <div style={{ fontSize: "11px", color: "rgba(255,255,255,0.5)", marginTop: "2px" }}>
                        {gapUSD > 1 ? "⚠ Not reported to accountant" : "✓ Balanced"}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {categoryList.length > 0 && (
                <div style={cardS}>
                  ...
                  {categoryList.map(([cat, totals]) => {
                    const totalAllCRC = totals.crc + totals.usd * monthRate;
      ...
      {activeTab === "bank" && !uploading && (
        <div>
          ...
          <button onClick={() => bankFileRef.current?.click()} style={{
            ...
            <span style={{ fontSize: "12px", color: B.textLight }}>BCR or LAFISE · XLS, CSV, or HTML format</span>
